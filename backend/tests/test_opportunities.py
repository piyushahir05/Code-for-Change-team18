...
def _make_recruiter(client, auth_as):
...
def test_create_opportunity_starts_pending(client, auth_as):
    headers, _ = _make_recruiter(client, auth_as)
...
def test_student_cannot_see_pending_opportunity(client, auth_as):
    recruiter_headers, _ = _make_recruiter(client, auth_as)
...
def test_recruiter_cannot_edit_another_recruiters_opportunity(client, auth_as):
    ...
    other_headers, _ = _make_recruiter(client, auth_as)
    response = client.put(
